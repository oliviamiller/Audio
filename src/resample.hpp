...
void resample_audio(int input_sample_rate, int output_sample_rate, int num_channels, const std::vector<uint8_t>& input_data, std::vector<uint8_t>& output_data) {
    ...
    soxr_error_t err = soxr_oneshot(
        ...
        num_channels,
        reinterpret_cast<int16_t*>(output_data.data()), input_samples, NULL,
        output_data.data(), output_samples, &output_done,
